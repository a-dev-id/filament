...
<x-desktop>
    ...
    <section>
        @foreach ($honeymoon_list as $key => $data)
        @if ($key % 2 == 0)
        <div class="py-5 bg-light-gray" id="{{$data->slug}}">
            @else
            <div class="py-5" id="{{$data->slug}}">
                @endif
                <div class="container">
                    <div class="row">
                        @if ($key % 2 == 0)
                        <div class="col-12 col-md-5 d-flex flex-column justify-content-between order-1">
                            ...
                            <div class="col-12 col-md-1 order-2 d-flex justify-content-center align-items-center">
                                <div class="d-flex flex-column align-items-center">
                                    ...
                                    <a onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=700');return false;" href="//www.facebook.com/sharer/sharer.php?u={{ route('honeymoon.index') }}" target="_blank" class="rounded-circle border-secondary text-secondary link-share link-share-facebook m-1"><i class="fa fa-facebook"></i></a>
                                    <a onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=700');return false;" href="//twitter.com/intent/tweet?text={{ $data->title }} - Experience at Nandini Jungle by Hanging gardens ({{ Str::limit($data->excerpt, 50) }}...Read more on our website!): {{ route('honeymoon.index') }}" target="_blank" class="rounded-circle border-secondary text-secondary link-share link-share-twitter m-1"><i class="fa fa-twitter"></i></a>
                                </div>
                            </div>
